...
function initializeEventListeners() {
  ...
  resultsPerPageDiv.addEventListener("click", event =>
    setResultsPerPage(event, searchProperties.resultsPerPageOptions)
  );
  ...
  createCollectionBtn.addEventListener("click", event =>
    openCreateRowModal(event, "collections")
  );
  createSourceBtn.addEventListener("click", event =>
    openCreateRowModal(event, "sources")
  );
  createEntryBtn.addEventListener("click", event =>
    openCreateRowModal(event, "entries")
  );
}
...
function insertSearchResults() {
  searchResultsDiv.innerHTML = getResultTableHTML(
    searchProperties,
    searchResultsData
  );
}
...
function scrollToTop(event) {
  if (event.target.nodeName === "BUTTON") {
    //only scroll to top if button was clicked, not random part of div
...
function insertResultsPerPageSelector() {
  if (searchProperties.totalResults !== 0) {
    resultsPerPageDiv.innerHTML =
      getResultsPerPageSelectorHTML(searchProperties);
...
function isMagnifyBtn(cellClicked) {
  ...
  return (
    cellClicked.classList.value.indexOf("btn-magnify") !== -1 || //SVG magnify button element triggered
    cellClicked.parentElement.classList.value.indexOf("btn-magnify") !== -1
  ); //USE element within SVG element triggered
}

function openSingleView(event) {
  let cellClicked = getTableCellClicked(event.target);
  let tableRow = cellClicked.parentElement;
  modalContentDiv.innerHTML = getSingleView(
    searchProperties.dataType,
    getRowData(tableRow)
  );
  openModal();
...
function displaySearchErrorMessage() {
  ...
  insertMarginHacks();
  resultsMessage.innerHTML =
    "There was an error with your search. Please check your connection and try again.";
}

function setResultsPerPage(event) {
  let clicked = event.target;
  if (clicked.className === "resultsPerPageLink") {
    //make sure link was clicked and not random part of page
    ...
    if (resultsPerPage == "All") {
      ...
      searchProperties.totalPages = Math.floor(
        searchProperties.totalResults / searchProperties.resultsPerPage + 1
      );
    }
    searchResultsDiv.innerHTML = getResultTableHTML(
      searchProperties,
...
function openEditorModal(event) {
  let cellClicked = getTableCellClicked(event.target);
  if (isEditableCell(cellClicked)) {
    //if cell clicked is not table header and not the id column
...
function getTableCellClicked(element) {
  ...
  if (nodeName === "PRE" || nodeName === "svg") {
    //some cells have PRE or SVG elements within, in which case parent cell must be returned
...
function constructEditorModal(cellClicked) {
  let tableRow = cellClicked.parentElement;
  modalContentDiv.innerHTML = getEditFormHTML(
    searchProperties.dataType,
    getRowData(tableRow)
  );
}
...
function getMatchingFormField(cellClicked) {
  ...
  return (
    Array.from(getEditRowForm().childNodes).find(row => row.id === clickedId) ||
    null
  );
}
...
function updateTableRow(event, tableRow) {
  ...
  xhr.open(
    "PUT",
    getHTTPRequestURL(getEditRowForm(), getTableSelection()),
    true
  );
...
function updateSearchResultsData(updatedRowData) {
  searchResultsData[
    searchResultsData.findIndex(data => data.id === updatedRowData.id)
  ] = updatedRowData;
}

//updated data being displayed
function updateSearchResultsDisplay(tableRow, updatedRowData) {
  tableRow.innerHTML = getTableRowHTML(
    searchProperties.dataType,
    updatedRowData,
    domainURL
  );
...
function displayUpdateRowErrorMessage(messageDiv) {
  messageDiv.innerHTML =
    "There was an error updating this row. Please check your connection and try again.";
}
...
function showDeleteRowPrompt(tableRow) {
  ...
  confirmDeleteBtn.addEventListener("click", event =>
    deleteTableRow(event, tableRow)
  );
  cancelDeleteBtn.addEventListener("click", () =>
    hideDeleteRowPrompt(actionDiv, tableRow)
  );
}
...
function displayDeleteRowErrorMessage(messageDiv) {
  messageDiv.innerHTML =
    "There was an error deleting this row. Please check your connection and try again.";
}
...
function selectResultPage(event) {
  ...
  if (btnClicked.nodeName === "BUTTON") {
    //make sure that button was clicked, and not "..." text
    if (btnClicked.innerText === "Next") {
      //if next button was clicked, increment page number
      searchProperties.curPage++;
    } else if (btnClicked.innerText === "Previous") {
      //if previous button was clicked, decrement page number
      searchProperties.curPage--;
    } else {
      //if page number was clicked, set page to page number
...
function addCreateRowEventListeners(dataType) {
  ...
  createRowBtn.addEventListener("click", event =>
    createTableRow(event, dataType)
  );
...
function createTableRow(event, dataType) {
  event.preventDefault();
  let modalMessageDiv = document.getElementById("modal-message");
  insertMiniLoadingSpinner(modalMessageDiv);
  let xhr = new XMLHttpRequest();
  ...
  xhr.onload = function () {
    if (requestSuccessful(this.status)) {
      clearCreateRowForm(dataType);
      //element has been recreated, so re-add new event listner
      modalMessageDiv = document.getElementById("modal-message");
      displayCreateRowSucces(modalMessageDiv);
    } else {
      displayCreateRowError(modalMessageDiv);
    }
  };
...
function displayCreateRowError(messageDiv) {
  messageDiv.innerHTML =
    "There was an error creating a new row. Please check your connection and try again.";
}
